...
newdat <- expand.grid(temp_mean = seq(min(dat$temp_mean, na.rm = TRUE) - 1 , 
                                      max(dat$temp_mean, na.rm = TRUE) + 1, 0.1),
                      nitrate_mean = seq(min(dat$nitrate_mean, na.rm = TRUE) - 1, 
                                         max(dat$nitrate_mean, na.rm = TRUE) + 1, 0.02),
                      zoo_total_biomass  = mean(dat$zoo_total_biomass, na.rm = TRUE),
                      # zoo_total_biomass  = seq(min(dat$zoo_total_biomass, na.rm = TRUE), 
                      #                          max(dat$zoo_total_biomass, na.rm = TRUE), 0.1),
                      phyto_mean_biovol  = seq(min(dat$phyto_mean_biovol, na.rm = TRUE), 
                                               max(dat$phyto_mean_biovol, na.rm = TRUE), 1))
...
ggplot(slopes, aes(temp_mean, nitrate_mean)) +
  # geom_raster(aes(fill = slope)) +
  geom_tile(aes(fill = slope)) +
  # scale_fill_viridis_c(option = 'plasma', na.value="grey70") +
  scale_fill_viridis_c(option = 'C') +
  theme_bw(base_size = 18) +
  scale_x_continuous(limits = c(min(dat$temp_mean, na.rm = TRUE) - 0.5, 
                                max(dat$temp_mean, na.rm = TRUE) + 0.5), expand = c(0, 0)) +
  scale_y_continuous(limits = c(min(dat$nitrate_mean, na.rm = TRUE) - 0.5, 
